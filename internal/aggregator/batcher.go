...
func (b *Batcher) ApplyChange(txID string, change KVChange) []Batch {
    ...
    if len(state.current) >= b.MaxBatchSize {
        return []Batch{b.emitLocked(txID, state, false)}
    }
    return nil
}
...
func (b *Batcher) Commit(txID string) []Batch {
    ...
    if len(state.current) > 0 {
        batches = append(batches, b.emitLocked(txID, state, true))
    }
...
func (b *Batcher) emitLocked(txID string, state *txBatchState, final bool) Batch {
